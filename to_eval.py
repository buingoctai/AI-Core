...
label_dic = {'greet': '4.0', 'findRestaurantsByCity': '3.0',
             'bye': '2.0', 'affirmative': '1.0', 'negative': '0.0'}
...
for i in range(0, 5):
    if(i == 0):
        ...
        result_str = "greet"
    score_dic.append(
        {'confidence': scores_nu[i], 'key': i, 'intent': result_str})
score_dic.sort(key=operator.itemgetter('confidence'), reverse=True)
print()
print("---------------------------Print Result of evaluating--------------------------------")
intent = {"name": score_dic[0]['intent'],
          "confidence": score_dic[0]['confidence']}
intent_ranking = [{'name': score_dic[0]['intent'], 'confidence':score_dic[0]['confidence']}, {'name': score_dic[1]['intent'], 'confidence': score_dic[1]['confidence']}, {'name': score_dic[2]['intent'],
